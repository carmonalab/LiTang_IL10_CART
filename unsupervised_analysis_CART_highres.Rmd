---
title: Analysis of MC38-HER2 CAR-T cells (high-resolution clustering) 
...
```{r}
data.projected <- ProjecTILs:::merge.Seurat.embeddings(data.list[[1]],data.list[[2]])
...
ggsave("plots/unsup_clustering_projtils_annot.pdf", height=4, width=6)
```
Include ProjecTILs annotations in clusters
```{r}
data.seurat$cluster.pt <- as.character(data.seurat$cluster)
data.seurat$cluster.pt[data.seurat$cluster=="0" & data.seurat$functional.cluster=="CD8_Tpex"] <- "0_Tpex"
data.seurat$cluster.pt[data.seurat$cluster=="0" & data.seurat$functional.cluster %in% c("CD8_NaiveLike","CD8_EarlyActiv")] <- "0_Tnaive"
#data.seurat$cluster.pt[data.seurat$cluster=="0" & data.seurat$functional.cluster %in% c("CD8_EffectorMemory")] <- "0_Tem"

table(data.seurat$cluster.pt)

data.seurat$cluster.pt <- factor(data.seurat$cluster.pt, levels= names(table(data.seurat$cluster.pt))) 
Idents(data.seurat) <- "cluster.pt"

ext.palette <- c("#FFD92F","#00B6EB","#A58AFF","#FC8D62","#B3B3B3","#8DA0CB")

g <- DimPlot(data.seurat, reduction = "umap", group.by = "cluster.pt", pt.size = 0.5, cols=ext.palette, label = T) + 
     NoLegend() + ggtitle("Unsupervised clusters") + theme(aspect.ratio=1.1)
g

ggsave("plots/unsup_clustering_pt.pdf", plot=g, height=4, width=6)
```
Frequency of cells by cluster and sample
```{r}
tab <- table(data.seurat$condition, data.seurat$cluster.pt)
tab.norm <- apply(tab, 1, function(x){x/(sum(x))})
...
ggplot(to.plot, aes(fill=Cluster, y=Frequency, x=Condition)) + 
    geom_bar(position="stack", stat="identity") +
  theme(axis.text.x = element_text(angle = 45)) +
  scale_fill_manual(values=ext.palette) + theme_bw()

ggsave("plots/unsup_cluster_frequency.pt.pdf", height = 5, width=3)


```

```{r}
feats1 <- c("Gzmb","Gzmc","Gzmf","Prf1","Pdcd1","Havcr2","Cd69")
feats2 <- c("Tcf7","S1pr1","Tnfsf4","Ccr7","Xcl1","Tox","Top2a","Mki67") 


d1 <- DotPlot(data.seurat,features = feats1, scale=F, group.by = "cluster.pt") + 
       theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + NoLegend()
d2 <- DotPlot(data.seurat,features = feats2, scale=F, group.by = "cluster.pt") +
...
feats <- c("Tcf7","S1pr1","Ccr7","Xcl1","Tnfsf4","Tox","Tigit","Lag3","Pdcd1","Gzmb","Gzmc","Gzmf","Prf1","Havcr2","Id2","Cd69","Mki67","Top2a","Mcm3","Mcm7")

names(ext.palette) <- names(table(data.seurat$cluster.pt))
h <- make.heatmap(data.seurat, genes=feats, palette=ext.palette, scale="row")
print(h)

ggsave("plots/heatmap_bulk_vertical.pdf", plot=h, height = 3, width=2.2)
```

Same but flipped
```{r fig.width=1.5, fig.height=1}
h <- make.heatmap(data.seurat, genes=feats, palette=ext.palette, scale="column", flip=T, method = "none")
print(h)

ggsave("plots/heatmap_bulk_horizontal.pdf", plot=h, height = 2.5, width=4)
```
...
ggsave("plots/volcano_cluster.pt_0_vs_1.pdf", height=8, width=10)
```
...
#Cluster 0
sub <- subset(data.seurat, subset=cluster.pt=="0")
...
#Cluster 1
sub <- subset(data.seurat, subset=cluster.pt=="1")
...
#Cluster 2
sub <- subset(data.seurat, subset=cluster.pt=="2")
...
#Cluster 3
sub <- subset(data.seurat, subset=cluster.pt=="3")
...
Compare predicted Tex (and Tpex?) between WT and IL10 cells
```{r}
library(EnhancedVolcano)
...
#Tex
sub <- subset(data.seurat, subset=functional.cluster=="CD8_Tex")

markers <- FindMarkers(sub, min.pct = 0.1, min.diff.pct=0.05, ident.1 = "IL10", ident.2 = "WT",
                                  logfc.threshold = 0.05, max.cells.per.ident = 500, test.use="wilcox",base=exp(1))

a <- EnhancedVolcano(markers, lab = rownames(markers), x = 'avg_logFC', y = 'p_val', FCcutoff=0.5, pCutoff = 10^(-6), title="Diff. expressed genes", subtitle = "IL10 vs. WT in CD8 Tex", drawConnectors = T, ylim = c(0,80))

#Tpex
sub <- subset(data.seurat, subset=functional.cluster=="CD8_Tpex")

markers <- FindMarkers(sub, min.pct = 0.1, min.diff.pct=0.05, ident.1 = "IL10", ident.2 = "WT",
                                  logfc.threshold = 0.05, max.cells.per.ident = 500, test.use="wilcox",base=exp(1))

b <- EnhancedVolcano(markers, lab = rownames(markers), x = 'avg_logFC', y = 'p_val', FCcutoff=0.5, pCutoff = 10^(-6), title="Diff. expressed genes", subtitle = "IL10 vs. WT in CD8 Tpex", drawConnectors = T, ylim = c(0,80))

a | b

ggsave("plots/il10vsWT.volcano.Tex.Tpex.pdf", height=9, width=14)
