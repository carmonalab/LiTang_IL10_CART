...
p <- FeaturePlot(data.seurat, features = c("Tcf7","Jun","Gzmb","Prf1","Pdcd1","Ifng","Ccr7","Fos","Gzmc","Gzmf","Havcr2","Mki67"), 
...
gsea.oxphos <- list("HALLMARK_OXIDATIVE_PHOSPHORYLATION" = gene.sets[["HALLMARK_OXIDATIVE_PHOSPHORYLATION"]]@geneIds)
...
signatures <- list("Cytotoxic"=c("Prf1","Gzmb","Gzmc","Gzmk","Gzmg"), 
...
wrap_plots(p, ncol=4)
ggsave("plots/umap_signatures.pdf",height=6, width=10)
...
p1 <- FeaturePlot(d1, features = c("Cytotoxic_UCell","Stemness_UCell","Exhaustion_UCell","HALLMARK_OXIDATIVE_PHOSPHORYLATION_UCell","Cycling_UCell"), cols=palette2, max.cutoff = 'q99', min.cutoff = 'q1', ncol = 5, combine = F)

d2 <- subset(data.seurat, subset=condition == "IL10")
p2 <- FeaturePlot(d2, features = c("Cytotoxic_UCell","Stemness_UCell","Exhaustion_UCell","HALLMARK_OXIDATIVE_PHOSPHORYLATION_UCell","Cycling_UCell"), cols=palette2, max.cutoff = 'q99', min.cutoff = 'q1', ncol = 5, combine = F)
...
ggsave("plots/umap_signatures.splitbysample.pdf",height=6, width=15)
...
feats1 <- c("Pdcd1","Havcr2","Gzmb","Gzmc","Prf1","Gzmf","Jun","Fos","Ifng","Mki67")
feats2 <- c("Aldh2","Gdf15","Akr1b8","Ndufv1","Sdhb","Sdhc","Cox5a","Cox15","Atp6v0a2") 
...
ggsave("plots/vlnplots_bycluster_stacked.pdf", height=6, width=6)


feats <- c(feats1, feats2)
vs <- VlnPlot(data.sub, features = feats, group.by = "cluster", pt.size = 0, idents=c("0","1"), cols = palette.samples, combine=F)
...
ggsave("plots/vlnplots_signature_bycluster.pdf", height=5, width=10)
```
