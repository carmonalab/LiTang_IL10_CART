...
#cpalette <- c("#ff7c79","#e7ed00","#0dcd00","#0f0f0f","#6596ff","#d574fb", "#cacaca")
cpalette <- c("#ff7c79","#0dcd00","#0f0f0f","#6596ff","#d574fb", "#cacaca")

g <- DimPlot(data.seurat, reduction = "umap", group.by = "cluster.pt", pt.size = 0.5, cols=cpalette, label = T) + 
     NoLegend() + ggtitle("Unsupervised clusters") + theme(aspect.ratio=1.1)
g
...
feats2 <- c("Tcf7","S1pr1","Id3","Ccr7","Xcl1","Tox","Mki67") 
...
DoHeatmap(data.seurat, features=feats, group.by = 'cluster.pt')
```

```{r}
library(dittoSeq)

dittoHeatmap(data.seurat, genes = feats,
             annot.by = c("cluster.pt"),
             cluster_cols = F,
             scaled.to.max = F,
             assay="RNA",
             slot="scale.data",
...
feats <- c("Tcf7","S1pr1","Ccr7","Xcl1","Tox","Gzmb","Gzmc","Gzmf","Prf1","Pdcd1","Havcr2","Lag3","Tigit","Id2","Id3","Cd69","Mki67","Mcm3","Mcm7")

palette <- c("#ff7c79","#0dcd00","#0f0f0f","#6596ff","#d574fb", "#cacaca")
#palette <- c("#ff7c79","#e7ed00","#0dcd00","#0f0f0f","#6596ff","#d574fb", "#cacaca")

names(palette) <- names(table(data.seurat$cluster.pt))
h <- make.heatmap(data.seurat, genes=feats, palette=palette, scale="row")
print(h)
```
...
#Increase resolution within cluster 0
...
data.seurat <- FindSubCluster(
  data.seurat,
  cluster="0",
  subcluster.name = "hr.cluster",
  resolution = res.sub,
  graph.name = "RNA_snn"
)

library(scales)
nclusters <- length(levels(data.seurat$hr.cluster))

g <- DimPlot(data.seurat, reduction = "umap", group.by = "hr.cluster", pt.size = 0.5, label = T) + 
     NoLegend() + ggtitle("Unsupervised clusters") + theme(aspect.ratio=1.1)
g
...
```{r}
data.seurat$cluster.2 <- as.character(data.seurat$cluster)

data.seurat$cluster.2[data.seurat$hr.cluster=="0_6"] = "Tpex"
data.seurat$cluster.2[data.seurat$hr.cluster=="0_8"] = "Tnaive"

data.seurat$cluster.2 <- factor(data.seurat$cluster.2, levels= names(table(data.seurat$cluster.2))) 
Idents(data.seurat) <- "cluster.2"

library(scales)
nclusters <- length(levels(data.seurat$cluster.2))
#cpalette <- hue_pal()(nclusters)

cpalette <- c("#ff7c79","#6596ff","#d574fb", "#cacaca", "#0dcd00","#0f0f0f")

g <- DimPlot(data.seurat, reduction = "umap", group.by = "cluster.2", pt.size = 0.5, cols=cpalette, label = T) + 
     NoLegend() + ggtitle("Unsupervised clusters") + theme(aspect.ratio=1.1)
g

ggsave("plots/unsup_clustering_hr.pdf", plot=g, height=4, width=6)
```

```{r}
feats1 <- c("Pdcd1","Havcr2","Gzmb","Gzmc","Prf1","Gzmf","Mki67")
feats2 <- c("Tcf7","S1pr1","Ccr7","Xcl1","Cd44","Cd69","Il7r") 


d1 <- DotPlot(data.seurat,features = feats1, scale=F, group.by = "cluster.2") + 
       theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + NoLegend()
d2 <- DotPlot(data.seurat,features = feats2, scale=F, group.by = "cluster.2") +
       theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
